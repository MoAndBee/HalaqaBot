...
export const addUserToList = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query("userLists")
            .withIndex("by_chat_post_user", (q) => q
            .eq("chatId", args.chatId)
            .eq("postId", args.postId)
        ...
        if (allUsers.length === 0) {
            ...
            if (postsInChat.length > 0) {
                ...
                const incompleteUsers = await ctx.db
                    .query("userLists")
                    .withIndex("by_chat_post", (q) => q.eq("chatId", args.chatId).eq("postId", previousPostId))
                    .collect();
        ...
        const updatedUsers = await ctx.db
            ...
            .withIndex("by_chat_post", (q) => q.eq("chatId", args.chatId).eq("postId", args.postId))
            .collect();
        const maxPosition = updatedUsers.length > 0
...
export const updateUserPosition = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const currentUser = await ctx.db
            ...
            .first();
        if (!currentUser) {
            throw new Error(`User ${args.userId} not found in list for chat ${args.chatId}, post ${args.postId}`);
        }
        ...
        if (args.newPosition < currentPosition) {
            ...
            const usersToShift = allUsers.filter((u) => u.position >= args.newPosition &&
            ...
            const usersToShift = allUsers.filter((u) => u.position > currentPosition &&
...
export const skipUserTurn = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!currentUser) {
            throw new Error(`User ${args.userId} not found or already completed their turn`);
        }
...
export const updateSessionType = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!user.completedAt) {
            throw new Error(`Cannot update session type for user ${args.userId} - turn not completed yet`);
        }
        // Update session type
        await ctx.db.patch(user._id, {
            sessionType: args.sessionType,
