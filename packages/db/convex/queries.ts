...
// Internal: paginates through userLists for the backfill action
export const getUserListsBatch = internalQuery({
  args: { cursor: v.union(v.string(), v.null()) },
  handler: async (ctx, args) => {
    return await ctx.db
      .query("userLists")
...
// Internal: paginates through posts for the duplicate cleanup action
export const getPostsBatch = internalQuery({
  args: { cursor: v.union(v.string(), v.null()) },
  handler: async (ctx, args) => {
    return await ctx.db
      .query("posts")
